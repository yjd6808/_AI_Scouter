...
async function Main(): Promise<void>
{
	...
	if (!Args.Safe)
	{
		const builtIn = Paths.IsPackaged ? undefined : "Source/Scouter.App/Renderer/BuiltIn";
		await PluginManager.LoadAllAsync(builtIn ?? null);
		PluginWatcher.Start([builtIn ?? "", userPlugins, Args.PluginDir ?? ""].filter((_d) => _d.length > 0));
	}
	else
	{
